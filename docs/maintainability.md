...
- Accept duplication when abstraction would increase complexity: two similar but conceptually different functions are better kept separate.
- Extract each shared unit with one clear responsibility; do not build a utility dumping ground.
...
Action: refactor incrementally when you encounter these smells. Use automated tools (SonarQube, `golangci-lint`, Ruff) to detect them.
